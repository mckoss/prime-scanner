...
[`submit/edits.yaml`](submit/edits.yaml) is the live list of what can be
contributed, generated by `oeis_audit.py` -- never edit it; record progress in
[`submissions.txt`](submissions.txt) and keep notes in [`NOTES.md`](NOTES.md).

## What is in this directory

Four kinds of file live here, and the difference matters -- the same terms
appear in three of them for different reasons:

| | what it is | edited by |
|---|---|---|
| `*.txt` here | our committed snapshot of a **published** b-file, in `n a(n)` form. `check_oeis.py` diffs the scan against these | by hand, when OEIS gains terms |
...
list, with every item's current state, is
[`submit/edits.yaml`](submit/edits.yaml):

1. **A096265 b-file**, 55 → 68 terms: [`submit/b096265.txt`](submit/b096265.txt).
...
3. **New sequence: balanced-lonely primes**: `new_sequences` in
   [`submit/edits.yaml`](submit/edits.yaml).
