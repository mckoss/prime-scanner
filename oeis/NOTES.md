...
Hand-kept. [`TODO.md`](TODO.md) is generated by `oeis_audit.py` and rewritten
on every run; this file is not, so reasoning, caveats and submission advice
belong here. Tag a section with an item's id -- `bound:lonely`, `new:balanced`
-- and the generated TODO links that item to its notes.
...
that date, and the live state is in `TODO.md` and `submissions.txt`.
